    ...
    var GameClient = Class.extend({
        ...
        connect: function(dispatcherMode) {
            var protocol = window.location.protocol === "https:" ? "https://" : "http://",
                ...
                url = protocol + this.host + port + "/",
                self = this;

            log.info("Trying to connect to server : "+url);

           this.connection = io(url, {forceNew: true, reconnection: false, timeout: 5000});// This sets the connection as a socket.io Socket.
            this.connection.on('connect_error', function(e) {
                log.error(e, true);
                if(self.fail_callback) {
                    self.fail_callback('serverunavailable');
                }
            });
            this.connection.on('connect_timeout', function(e) {
                log.error(e, true);
                if(self.fail_callback) {
                    self.fail_callback('serverunavailable');
                }
