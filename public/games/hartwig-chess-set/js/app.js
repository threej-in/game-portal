...
function getTransformValue(style) {
...
function getTranslateFromMatrix(transformValue) {
  if (!transformValue || transformValue === "none") {
    return { x: 0, y: 0 };
  }

  var match2d = transformValue.match(/^matrix\((.+)\)$/);
  if (match2d) {
    var values2d = match2d[1].split(/\s*,\s*/).map(parseFloat);
    return {
      x: values2d[4] || 0,
      y: values2d[5] || 0
    };
  ...
  var match3d = transformValue.match(/^matrix3d\((.+)\)$/);
  if (match3d) {
    var values3d = match3d[1].split(/\s*,\s*/).map(parseFloat);
    return {
      x: values3d[12] || 0,
      y: values3d[13] || 0
    };
...
function grabPiece(event) {
  if (!mouseDown && controls) {
    ...
    startX = pointer.x - (document.body.offsetWidth/2);
    startY = pointer.y - (document.body.offsetHeight/2);
    style = window.getComputedStyle(grabbed);
    matrix = getTransformValue(style);
    matrixParts = getTranslateFromMatrix(matrix);
    grabbedW = parseInt(style.getPropertyValue('width'))/2;
    grabbedX = matrixParts.x;
    grabbedY = matrixParts.y;
...
function dragPiece(event) {
  if (mouseDown && controls) {
    event.preventDefault();
    var pointer = getPointerPosition(event);
    moveX = pointer.x - (document.body.offsetWidth/2);
    moveY = pointer.y - (document.body.offsetHeight/2);
    distX = moveX-startX;
    distY = moveY-startY;
    if (currentColor === "w") {
      newX  = grabbedX+distX;
...
function moveScene(event) {
  if (animated) {
    var animatedPointer = getPointerPosition(event);
    eventStartX = animatedPointer.x - (document.body.offsetWidth/2);
    eventStartY = animatedPointer.y - (document.body.offsetHeight/2);
  }
  eventStartX = 0;
  eventStartY = 0;
  if (!controls && !animated) {
    ...
    event.preventDefault();
    var pointer = getPointerPosition(event);
    eventMoveX = pointer.x - (document.body.offsetWidth/2);
    eventDistX = (eventMoveX - eventStartX);
    eventMoveY = pointer.y - (document.body.offsetHeight/2);
